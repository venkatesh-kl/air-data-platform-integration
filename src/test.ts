...
async function loadDataForOrg(orgId: string): Promise<void> {
	const dataSources = await AIRDPModel.getDataSources(orgId);
	saveJSONFile(`${orgId}/dataSources`, dataSources);

	for (const dataSource of dataSources) {
		const sampleData = await AIRDPModel.getPreviewData(dataSource.orgId, dataSource.product, dataSource.schema);
		saveJSONFile(`${dataSource.orgId}/${dataSource.product}/${dataSource.schema}/preview`, sampleData);
	}
}
